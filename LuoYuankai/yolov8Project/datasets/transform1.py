...
if __name__ == '__main__':
    ...
    outpath_txt = 'E:/dataset/WiderPerson/label/train'   
    # 注意：这里F:/dataset/WiderPerson是你存储文件侧地方，可以替换，后面的不要动
    outpath_jpg = 'E:/dataset/WiderPerson/images/train'
    # 注意：这里F:/dataset/WiderPerson是你存储文件侧地方，可以替换，后面的不要动
    os.makedirs(outpath_txt)
    os.makedirs(outpath_jpg)

    path = 'E:/dataset/WiderPerson/train.txt'
    with open(path, 'r') as f:
        img_ids = [x for x in f.read().splitlines()]

    for img_id in img_ids:  # '000040'
        img_path = 'E:/dataset/WiderPerson/Images/' + img_id + '.jpg'
